#!/usr/bin/env python3
"""
Convert HDF5 dataset to numpy memmap format for memory-efficient fast training.
Memmap allows memory-mapped file access - data stays on disk, loaded on demand.
"""
...
def convert_hdf5_to_mmap(input_dir: str, output_dir: str):
    """Convert all HDF5 files to numpy memmap format."""
    ...
    for hdf5_file in tqdm(hdf5_files, desc="Scanning"):
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=str, default="data/dual_arm_demos",
                        help="Input directory containing HDF5 files")
    parser.add_argument("--output", type=str, default="data/dual_arm_demos_mmap",
                        help="Output directory for memmap files")
    args = parser.parse_args()
    
    convert_hdf5_to_mmap(args.input, args.output)
